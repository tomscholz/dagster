...
from .asset_condition import AssetConditionEvaluationInfo, AssetConditionEvaluationResult
...
class AssetDaemonContext:
    ...
    def evaluate_asset(
    ...
    ) -> Tuple[AssetConditionEvaluationInfo, Optional[datetime.datetime]]:
        ...
        asset_cursor = self.cursor.get_previous_evaluation_info(asset_key)

        context = AssetConditionEvaluationContext.create(
            asset_key=asset_key,
            previous_evaluation_info=asset_cursor,
        ...
        evaluation_info = asset_condition.evaluate(context)

        expected_data_time = get_expected_data_time_for_asset_key(
            context, will_materialize=evaluation_info.true_subset.size > 0
        )
        return evaluation_info, expected_data_time

    def get_asset_condition_evaluations(
        self,
    ) -> Tuple[Sequence[AssetConditionEvaluationInfo], AbstractSet[AssetKeyPartitionKey]]:
        ...
        """
        evaluation_info_by_key: Dict[AssetKey, AssetConditionEvaluationInfo] = {}
        ...
        for asset_key in itertools.chain(*self.asset_graph.toposort_asset_keys()):
            ...
            (evaluation_info, expected_data_time) = self.evaluate_asset(
                asset_key, evaluation_info_by_key, expected_data_time_mapping
            )

            num_requested = evaluation_info.true_subset.size
            log_fn = self._logger.info if num_requested > 0 else self._logger.debug

            to_request_asset_partitions = evaluation_info.true_subset.asset_partitions
            to_request_str = ",".join(
            ...
            evaluation_info_by_key[asset_key] = evaluation_info
            ...
            if num_requested > 0:
                for neighbor_key in self.asset_graph.get_required_multi_asset_keys(asset_key):
                    ...
                    if neighbor_key in evaluation_info_by_key:
                        neighbor_evaluation = evaluation_info_by_key[neighbor_key]
                        evaluation_info_by_key[neighbor_key] = neighbor_evaluation._replace(
                            evaluation_result=neighbor_evaluation.evaluation_result._replace(
                    ...
                    to_request |= {
                        ...
                        for ap in evaluation_info.true_subset.asset_partitions
                    }

        return (list(evaluation_info_by_key.values()), to_request)
    ...
    def evaluate(
        ...
        evaluation_info, to_request = self.get_asset_condition_evaluations()
        ...
        return (
            ...
            [
                ei.evaluation_result
                for ei in evaluation_info
                if not ei.evaluation_result.equivalent_to_stored_evaluation(
                    self.cursor.get_previous_evaluation_result(ei.asset_key)
                )
            ],
        )
