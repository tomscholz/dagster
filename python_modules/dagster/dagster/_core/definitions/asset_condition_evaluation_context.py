...
class AssetConditionEvaluationContext:
    ...
    previous_evaluation_info: Optional["AssetConditionEvaluationInfo"]
    previous_evaluation_result: Optional["AssetConditionEvaluationResult"]
    ...
    evaluation_info_by_key: Mapping[AssetKey, "AssetConditionEvaluationInfo"]
    ...
    def create(
        ...
        return AssetConditionEvaluationContext(
            ...
            previous_evaluation_result=previous_evaluation_info.evaluation_result
            if previous_evaluation_info
            else None,
            ...
            evaluation_info_by_key=evaluation_info_by_key,
    ...
    def for_child(
        ...
        return dataclasses.replace(
            self,
            condition=condition,
            previous_evaluation_result=self.previous_evaluation_result.for_child(condition)
            if self.previous_evaluation_result
            else None,
    ...
    def previous_max_storage_id(self) -> Optional[int]:
        return (
            self.previous_evaluation_info.max_storage_id if self.previous_evaluation_info else None
        )

    @property
    def previous_evaluation_timestamp(self) -> Optional[float]:
        return self.previous_evaluation_info.timestamp if self.previous_evaluation_info else None

    @property
    def previous_true_subset(self) -> AssetSubset:
        if self.previous_evaluation_result is None:
            return self.empty_subset()
        return self.previous_evaluation_result.true_subset

    @property
    def previous_subsets_with_metadata(self) -> Sequence["AssetSubsetWithMetadata"]:
        if self.previous_evaluation_result is None:
            return []
        return self.previous_evaluation_result.subsets_with_metadata
    ...
    def parent_will_update_subset(self) -> AssetSubset:
        ...
        for parent_key in self.asset_graph.get_parents(self.asset_key):
            if not self.materializable_in_same_run(self.asset_key, parent_key):
                continue
            parent_info = self.evaluation_info_by_key.get(parent_key)
            if not parent_info:
                continue
    ...
    def previous_tick_requested_subset(self) -> AssetSubset:
        ...
        if (
            self.previous_evaluation_info is None
            or self.previous_evaluation_info.evaluation_result is None
        ):
            return self.empty_subset()

        return self.previous_evaluation_info.evaluation_result.get_requested_or_discarded_subset(
            self.condition
        )
    ...
    def candidates_not_evaluated_on_previous_tick_subset(self) -> AssetSubset:
        ...
        tick.
        """
        from .asset_condition import HistoricalAllPartitionsSubset

        if not self.previous_evaluation_result:
            return self.candidate_subset
        # when the candidate_subset is HistoricalAllPartitionsSubset, this indicates that the
        ...
        # were *not* evaluated on the previous tick
            ...
            self.previous_evaluation_result.candidate_subset, HistoricalAllPartitionsSubset
        ):
            return self.empty_subset()
        return self.candidate_subset - self.previous_evaluation_result.candidate_subset
    ...
    def will_update_asset_partition(self, asset_partition: AssetKeyPartitionKey) -> bool:
        parent_evaluation_info = self.evaluation_info_by_key.get(asset_partition.asset_key)
        if not parent_evaluation_info:
            return False
        return asset_partition in parent_evaluation_info.true_subset
