...
def observe(
    ...
    *,
    ...
    """
    assets = normalize_renamed_param(assets, "assets", source_assets, "source_assets")
    assets = check.sequence_param(assets, "assets", of_type=(SourceAsset, AssetsDefinition))
    instance = check.opt_inst_param(instance, "instance", DagsterInstance)
    partition_key = check.opt_str_param(partition_key, "partition_key")
    resources = check.opt_mapping_param(resources, "resources", key_type=str)

    external_assets = [x for x in source_assets if isinstance(x, AssetsDefinition)]
    source_assets = [x for x in source_assets if isinstance(x, SourceAsset)]
